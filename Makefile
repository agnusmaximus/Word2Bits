...
benchmark-mikolov:
	...
	$(BUILDDIR)/word2bits_mikolov -min-count $(MIN_COUNT) -bitlevel ${BITLEVEL} -train $(TEXT8_PATH) -output $(MIKOLOV_SAVE_FILE)_text8_bitlevel${BITLEVEL}_size${VECTOR_SIZE}_window${WINDOW_SIZE}_neg${NEGATIVE_SIZE}_reg${REG}_maxiter${MAX_ITER}_mincount${MIN_COUNT} -size $(VECTOR_SIZE) -window $(WINDOW_SIZE) -negative $(NEGATIVE_SIZE) -sample 1e-4 -threads $(NUM_THREADS) -binary 1 -iter $(MAX_ITER) -reg $(REG)

	@echo Evaluating Mikolov
	$(BUILDDIR)/compute_accuracy_mikolov $(MIKOLOV_SAVE_FILE)_text8_bitlevel${BITLEVEL}_size${VECTOR_SIZE}_window${WINDOW_SIZE}_neg${NEGATIVE_SIZE}_reg${REG}_maxiter${MAX_ITER}_mincount${MIN_COUNT} < data/google_analogies_test_set/questions-words.txt
benchmark-mikolov-large:
	...
	rm -f ${FILEOUT}
	$(BUILDDIR)/word2bits_mikolov -min-count $(MIN_COUNT_FULL) -bitlevel ${BITLEVEL} -train $(FULL_WIKI_PATH) -output $(FULLWIKI_MIKOLOV_SAVE_FILE)_fullwiki_bitlevel${BITLEVEL}_size${VECTOR_SIZE}_window${WINDOW_SIZE}_neg${NEGATIVE_SIZE}_reg${REG}_maxiter${MAX_ITER}_mincount${MIN_COUNT_FULL} -size $(VECTOR_SIZE) -window $(WINDOW_SIZE) -negative $(NEGATIVE_SIZE) -sample 1e-4 -threads $(NUM_THREADS) -binary 1 -iter $(MAX_ITER) -reg $(REG) >> ${FILEOUT} 
