...
void *TrainModelThread(void *id) {
  ...
  while (1) {
	...
	if (last_word == -1) continue;
	  for (c = 0; c < layer1_size; c++) context_avg[c] += u[c + last_word * layer1_size];
	  cw++;
      }
    if (cw) {
	...
	f = 0;
	for (c = 0; c < layer1_size; c++) f += context_avg[c] * v[c + l2];
	if (f > MAX_EXP) g = (label - 1) * alpha;
	else if (f < -MAX_EXP) g = (label - 0) * alpha;
	else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
	for (c = 0; c < layer1_size; c++) context_avge[c] += g * v[c + l2];
	for (c = 0; c < layer1_size; c++) v[c + l2] += g * context_avg[c];
      }
      // hidden -> in
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
	  c = sentence_position - window + a;
	  ...
	  last_word = sen[c];
	  if (last_word == -1) continue;
	  for (c = 0; c < layer1_size; c++) u[c + last_word * layer1_size] += context_avge[c];
	}
    }
    sentence_position++;
...
void TrainModel() {
  ...
  if (classes == 0) {
    ...
    for (a = 0; a < vocab_size; a++) {
      ...
      for (b = 0; b < layer1_size; b++) {
	real avg = u[a*layer1_size+b] + v[a*layer1_size+b];
	if (binary) fwrite(&avg, sizeof(real), 1, fo);
